...
def get_arguments():
    ...
    parser.add_argument("--input", type=str, default='7e1922575c_1')
...
for i in range(5):
    #### rgb
    img_file = os.path.join('Image_inputs', seq_name, 'gt_{:1d}.jpg'.format(i))
    ...
    frames[i] = raw_frame
    #### mask
    mask_file = os.path.join('Image_inputs', seq_name, 'mask_{:1d}.png'.format(i))
    ...
    #### dist 应用领域：如果应用到单幅图像上则是主要为图像窄化、骨架提取、、粘连物体的分离等；如果应用到原图跟模板图则是应用于图像匹配。
